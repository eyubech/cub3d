...
/*   Updated: 2023/08/25 19:11:36 by nel-mous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub.h"

void	cast_horizontal(t_cub_data *cb_data, float player_x,
	float player_y, int colomn)
{
	float	delta_x;
	float	delta_y;
...
void	cast_vertical(t_cub_data *cb_data, float player_x,
	float player_y, int colomn)
{
	float	delta_x;
	float	delta_y;
...
void	compare_distance(t_cub_data *cb_data, float player_x,
	float player_y, int colomn)
{
	...
	cb_data->sr[colomn].ver_dist = sqrt(pow((player_x - 
					cb_data->sr[colomn].vx_inter), 2) 
			+ pow((player_y - cb_data->sr[colomn].vy_inter), 2));
		// draw_map(cb_data);
